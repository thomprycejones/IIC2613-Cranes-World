...
container(C) :- member(C,[c1,c2,c3,c4,c5]).
paleta(P) :- member(P,[p11,p21,p12,p22]).
superficie(S) :- container(S).
superficie(S) :- paleta(S).
lugar(L) :- member(L,[cargo1,cargo2]).
conectada(C1,C2) :- member([X1,X2],[[cargo1,cargo2]]),
                    (C1=X1,C2=X2; C1=X2,C2=X1).

grua(G) :- member(G,[g1,g2]).

camion(Cam) :- member(Cam,[cam1, cam2]).

%% Initial Situation (problem-specific)
holds(F,s0) :- member(F,[en(cam1,cargo1), en(cam2, cargo2),

                         en(g1,cargo1),en(g2,cargo2),
...
%goal_condition([en(c2,cargo2),en(c4,cargo1)]).
%goal_condition([sobre(c4,p21)]).
%goal_condition([dentro(c3,cam1)]).
%goal_condition([dentro(c1,cam2), dentro(c4,cam1)]).
goal_condition([en(c1,cargo2),sobre(c3,p22)]).
